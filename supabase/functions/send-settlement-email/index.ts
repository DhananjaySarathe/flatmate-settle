  ...
  try {
    ...
    const emailPromises = flatmatesWithEmails.map(async (flatmate) => {
      ...
      if (emailType === 'individual') {
        ...
        subject = `Your ExpenseWaale Settlement Summary - ${reportData.dateRange.from} to ${reportData.dateRange.to}`
        
        htmlContent = generateIndividualEmailHTML(name, balance, settlements, reportData)
      } else {
        subject = `ExpenseWaale Settlement Report - ${reportData.dateRange.from} to ${reportData.dateRange.to}`
...
function generateIndividualEmailHTML(name: string, balance: any, settlements: any[], reportData: any): string {
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <title>Your ExpenseWaale Settlement</title>
      <style>
    ...
    <body>
      <div class="header">
        <h1>💰 ExpenseWaale Settlement Report</h1>
      ...
      <div class="footer">
        <p>Generated by ExpenseWaale Expense Management System</p>
...
function generateComprehensiveEmailHTML(reportData: any): string {
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <title>ExpenseWaale Settlement Report</title>
      <style>
