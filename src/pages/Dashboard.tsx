    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage your flatmates and view expense overview</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={(open) => {
          ...
          <DialogContent className="glass-card border-border/50">
            <DialogHeader>
              <DialogTitle>{editingFlatmate ? "Edit Flatmate" : "Add New Flatmate"}</DialogTitle>
              <DialogDescription>
                {editingFlatmate ? "Update flatmate information" : "Add a new flatmate to your group"}
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-gradient-to-r from-primary to-blue-500">
                {editingFlatmate ? "Update Flatmate" : "Add Flatmate"}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
        ...
        <CardContent>
          {flatmates.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">No flatmates added yet</p>
              <Button
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h3 className="font-semibold text-lg mb-1">{flatmate.name}</h3>
                          {flatmate.email && (
                            <p className="text-sm text-muted-foreground">{flatmate.email}</p>
                          )}
                          {flatmate.phone && (
                            <p className="text-sm text-muted-foreground">{flatmate.phone}</p>
                          )}
                        </div>
